...
_TRANSLATION_X_RE = re.compile(r"(?<![A-Za-z])x+(?![A-Za-z])")
_TRANSLATION_ELLIPSIS_RE = re.compile(r"(?:\.{3,}|…+)")
_FRACTION_GLYPH_RE = re.compile(r"([¼½⅓⅔⅕⅖⅗⅘⅙⅚⅛⅜⅝⅞¾⅒])")
_MIXED_FRAC_RE = re.compile(r"(?<!\d)(\d+)\s+(\d+)\s*/\s*(\d+)(?!\d)")
_COMPACT_FRAC_RE = re.compile(r"(?<!\d)(\d+)\s*/\s*(\d+)(?!\d)")
_SUPERSCRIPT_LIKE_CHARS = ("ᵖ", "ˡ", "ʳ", "ᵘ", "ʼ")
...
def _normalize_compact_fractions(text: str) -> str:
    """Normalize compact fractions like 1/3, 1 1/3 to Unicode glyphs."""
    frac_map = {
        (1, 2): "½", (1, 3): "⅓", (2, 3): "⅔",
        (1, 4): "¼", (3, 4): "¾",
        (1, 5): "⅕", (2, 5): "⅖", (3, 5): "⅗", (4, 5): "⅘",
    ...
    def repl_mixed(m: re.Match[str]) -> str:
        glyph = frac_map.get((int(m.group(2)), int(m.group(3))))
        return f"{m.group(1)} {glyph}" if glyph else m.group(0)

    def repl_simple(m: re.Match[str]) -> str:
        glyph = frac_map.get((int(m.group(1)), int(m.group(2))))
        return glyph if glyph else m.group(0)

    text = _MIXED_FRAC_RE.sub(repl_mixed, text)
    return _COMPACT_FRAC_RE.sub(repl_simple, text)
...
def normalize_translation(text: str) -> str:
    ...
    # --- Phase 0: Character normalization ---
    text = text.replace("\u2018", "'").replace("\u2019", "'")
    text = text.replace("\u201c", '"').replace("\u201d", '"')
    for ch in _SUPERSCRIPT_LIKE_CHARS:
        text = text.replace(ch, "")
    text = text.replace("*", "")
    text = _TRANSLATION_X_RE.sub("<gap>", text)

    # --- Phase 1: Pattern replacements ---
    ...
    text = _normalize_compact_fractions(text)
    char_trans = str.maketrans(char_map)
    text = text.translate(char_trans)

    # --- Phase 3: Gap & cleanup ---
    text = _TRANSLATION_ELLIPSIS_RE.sub(" <gap> ", text)
    ...
    text = _FRACTION_GLYPH_RE.sub(r" \1 ", text)
    text = re.sub(r"\s+([,.;:!?])", r"\1", text)
...
def assign_group_folds(
    ...
    """Assign fold indices so that all rows sharing the same group stay in one fold.

    Groups are shuffled, then greedily assigned to the fold with the fewest
    samples so far (bin-packing) to keep fold sizes balanced.
    """
    import numpy as np
    ...
    # Greedy bin-packing: assign each group to the fold with fewest rows
...
def build_train_processed(
    train_csv_path: str = "input/deep-past-initiative-machine-translation/train.csv",
    published_texts_csv_path: str = "input/deep-past-initiative-machine-translation/published_texts.csv",
    n_splits: int = 4,
    n_bins: int = 20,
    seed: int = 42,
) -> "pl.DataFrame":
    ...
    train = pl.read_csv(train_csv_path)
    published_texts = pl.read_csv(published_texts_csv_path, ignore_errors=True)
    published_texts_for_join = (
        published_texts.select(["oare_id", "transliteration"])
    ...
    train = (
        train.with_columns(
            ...
            .map_elements(normalize_transliteration, return_dtype=pl.String)
            .alias("transliteration_orig"),
            pl.col("translation").map_elements(
                normalize_translation, return_dtype=pl.String
            ),
        ...
        .with_columns(
            ...
            .otherwise(
                pl.coalesce([pl.col("published_texts"), pl.col("transliteration_orig")])
            )
    ...
    folds = assign_stratified_folds_from_continuous(
        train.get_column("length_ratio").to_list(),
        n_splits=n_splits,
        n_bins=n_bins,
        seed=seed,
    )
    cols = ["oare_id", "transliteration", "translation", "fold"]
    train = train.with_columns(pl.Series("fold", folds).cast(pl.Int8))
    return train.select(cols)
